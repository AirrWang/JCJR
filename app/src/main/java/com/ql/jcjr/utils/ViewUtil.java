...
public class ViewUtil {
    ...
    public static int scaleValue(Context context, float value) {
        ...
        if (mDisplayMetrics.scaledDensity > AppConfigCommon.UI_DENSITY) {
            if (mDisplayMetrics.widthPixels > AppConfigCommon.UI_WIDTH) {
                value = value * (1.3f - 1.0f / mDisplayMetrics.scaledDensity);
            } else if (mDisplayMetrics.widthPixels < AppConfigCommon.UI_WIDTH) {
    ...
    public static int scale(int displayWidth, int displayHeight, float pxValue) {
        ...
        float scale = 1;
        try {
            float scaleWidth = (float) displayWidth / AppConfigCommon.UI_WIDTH;
            float scaleHeight = (float) displayHeight / AppConfigCommon.UI_HEIGHT;
