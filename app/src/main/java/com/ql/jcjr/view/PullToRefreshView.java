    ...
    private boolean isRefreshViewScroll(int deltaY) {
        ...
        if (mRecyclerView != null) {
            if (deltaY > 0) {
                ...
                View lastChild = mRecyclerView.getChildAt(manager.getChildCount() - 1);
                ...
                // End scroll
                if (lastChild.getBottom() <= getHeight()
                        && manager.findLastCompletelyVisibleItemPosition() == mRecyclerView.getAdapter().getItemCount() - 1) {
                    mPullState = PULL_UP_STATE;
                    return true;
                }
            }
...
//            LogUtil.i("onFooterLoadFinish countNew： " + countNew);
            // adjust data is null
            if (countNew > mCount) {
                mFooterView.setState(RefreshViewFooter.STATE_READY);
            } else {
