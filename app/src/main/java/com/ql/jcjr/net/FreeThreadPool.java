...
public class FreeThreadPool {
    ...
    static ArrayBlockingQueue<Runnable> blockingQueue = new ArrayBlockingQueue<Runnable>(
            FreeThreadPool.BLOCKING_QUEUE_SIZE);
    /**
     * 线程池，目前是十个线程，
     */
    ...
    public static FreeThreadPool getInstance() {
        return Singleton.instance;
    }

    public static void removeAllTask() {
        FreeThreadPool.blockingQueue.clear();
    }

    public static void removeTaskFromQueue(final Object obj) {
        FreeThreadPool.blockingQueue.remove(obj);
    }
    ...
    public static void shutdown() {
        if (FreeThreadPool.pool != null) {
            FreeThreadPool.pool.shutdown();
        }
    }
    ...
    public static void shutdownRightnow() {
        if (FreeThreadPool.pool != null) {
            FreeThreadPool.pool.shutdownNow();
            try {
                // 设置超时极短，强制关闭所有任务
                FreeThreadPool.pool.awaitTermination(1,
    ...
    private static class Singleton {
        private static FreeThreadPool instance = new FreeThreadPool();
    }
}
