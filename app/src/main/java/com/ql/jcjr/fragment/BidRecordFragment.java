...
import com.ql.jcjr.adapter.BidRecordAdapter;
 ...
 */
public class BidRecordFragment extends BaseFragment implements AdapterView.OnItemClickListener,
    ...
    List<BidRecordEntity.ResultBean> mRecordList = new ArrayList<>();
    ...
    private void init() {
        initListVieww();
        getBidRecord(String.valueOf(mPageIndex));
    }

    private void initListVieww() {
        mAdapter = new BidRecordAdapter(mContext, mRecordList);
        mLvTransRecord.setAdapter(mAdapter);
        mLvTransRecord.setOnItemClickListener(this);
    }

    private void getBidRecord(final String pageIndex) {
        SenderResultModel resultModel = ParamsManager.senderBidRecord(pageIndex, "10");

        HttpRequestManager.httpRequestService(resultModel, new HttpSenderController.ViewSenderCallback() {

            @Override
            public void onSuccess(String responeJson) {
                LogUtil.i("投标记录 " + responeJson);
                BidRecordEntity entity = GsonParser.getParsedObj(responeJson, BidRecordEntity.class);
            ...
            public void onFailure(ResponseEntity entity) {
                LogUtil.i("投标记录失败 " + entity.errorInfo);
