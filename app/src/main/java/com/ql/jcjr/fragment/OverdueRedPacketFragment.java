 ...
 */
public class OverdueRedPacketFragment extends BaseFragment implements AdapterView.OnItemClickListener,
    ...
    private void init() {
        initListVieww();
        getOverdueCoupon(String.valueOf(mPageIndex));
    ...
    private void initListVieww() {
        mAdapter = new RedPacketAdapter(mContext, mList, Global.STATUS_OVERDUE);
        mLvAvailable.setAdapter(mAdapter);
        mLvAvailable.setOnItemClickListener(this);
    }

    private void getOverdueCoupon(final String pageIndex) {
        SenderResultModel resultModel = ParamsManager.senderMyCoupon(pageIndex, "10", Global.STATUS_OVERDUE);

        HttpRequestManager.httpRequestService(resultModel, new HttpSenderController.ViewSenderCallback() {

            @Override
            public void onSuccess(String responeJson) {
                LogUtil.i("过期红包 " + responeJson);
            ...
            public void onFailure(ResponseEntity entity) {
                LogUtil.i("过期红包失败 " + entity.errorInfo);
